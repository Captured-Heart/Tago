...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    //     .valueOrNull
    //     ?.where((element) => element.status! == 7)
    //     .toList();
    // log(HiveHelper().getData(HiveKeys.token.keys).toString());
    var deals = categoriesGroup.valueOrNull?.deals;
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                //         context: context,
                //         ref: ref,
                //         orderModel: orderList?[0] ?? OrderListModel(),
                //       ).padOnly(
                //         top: 10,
                //         bottom: 25,
                //       ),
                          ...
                          onTap: () {
                            if (deals != null) {
                              push(
                                context,
                                TagScreen(
                                  imageUrl: deals.map((e) => e.imageUrl).nonNulls.first,
                                  appBarTitle: deals.map((e) => e.name).nonNulls.first,
                                  tagId: deals.map((e) => e.id).nonNulls.first,
                                ),
                              );
                            }
                          },
